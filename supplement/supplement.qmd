...
load('staged_sim_results_supp.RData')
pcps_clean <- read_csv("../data/pcps_clean.csv")
lmt_clean <- read_csv("../data/lmt_clean.csv")
flanker_clean <- read_csv("../data/flanker_clean.csv")
dccs_clean <- read_csv("../data/dccs_clean.csv")
...
#| dpi: 600
#| fig-cap: | 
#|   **Figure S1. **Graphical overview of the data access workflow using R and GitHub.
knitr::include_graphics("images/figS1.png")
```
...
#| dpi: 600
#| fig-cap: | 
#|   **Figure S2. **Results of the power simulations. The dashed line indicates 90% power.
...
See Table S1 for descriptive statistics for all cognitive tasks.
...
flextable(staged_sim_results_supp$descriptives, cwidth = c(2, 1.2, 1.2, 1.2, 1.2)) |> 
  ...
  compose(
    i = 1, j = 1, 
    as_paragraph(as_b("Table S1. "), "Descriptive statistics of mean RTs and accuracy for the cognitive tasks."),
    part = "header"
  )
```
...
Third, we will assess whether the model provides a good fit to the participants' data using simulation (See Figure S3 for a visualization of this procedure).
...
However, estimates of boundary separation and non-decision time showed substantial bias (See Figure S5).
...
#| dpi: 600
#| fig-cap: | 
#|   **Figure S4. **Convergence of the model in simulation 1. Plots should resemble a 'fat, hairy caterpillar'.
...
#| dpi: 600
#| fig-cap: | 
#|   **Figure S5. **Parameter recovery in the case of two conditions. a = Boundary Separation; t = Non-Decision Time; v = Drift Rate.
...
The model converged normally (Figure S6). 
Correlations between simulated and recovered parameter estimates was high, ranging between *r* = `r staged_sim_results_supp$ddm_sim2_cor$v` for the drift rate and `r staged_sim_results_supp$ddm_sim2_cor$t` for the non-decision time (see Figure S7).
...
#| dpi: 600
#| fig-cap: | 
#|   **Figure S6. **Convergence of the model in simulation 2. Plots should resemble a 'fat, hairy caterpillar'.
...
#| dpi: 600
#| fig-cap: | 
#|   **Figure S7. **Parameter recovery in the case of two conditions. a = Boundary Separation; t = Non-Decision Time; v = Drift Rate
...
The model converged normally (Figure S8). 
Correlations between simulated and recovered parameter estimates were high and very similar to those found in simulation 2, ranging between *r* = `r staged_sim_results_supp$ddm_sim3_cor$v` for the drift rate and `r staged_sim_results_supp$ddm_sim3_cor$t` for the non-decision time (see Figure S9).
...
#| dpi: 600
#| fig-cap: | 
#|   **Figure S8. **Convergence of the model in simulation 3. Plots should resemble a 'fat, hairy caterpillar'.
...
#| dpi: 600
#| fig-cap: | 
#|   **Figure S9.**Parameter recovery in the case of two conditions. a = Boundary Separation; t = Non-Decision Time; v = Drift Rate
...
#| dpi: 600
#| fig-cap: | 
#|   **Figure S10.**Histograms of simulated and recovered parameter estimates. a = Boundary Separation; t = Non-Decision Time; v = Drift Rate.
...
#| dpi: 600
#| fig-cap: | 
#|   **Figure S11.**Deviation between simulated and recovered parameter estimates as a function of adversity. The regression lines show quadratic effects. a = Boundary Separation; t = Non-Decision Time; v = Drift Rate.
...
As Figure S12 illustrates, there seemed to be small shrinkage effects at the low and high levels of adversity.
...
#| dpi: 600
#| fig-cap: | 
#|   **Figure S12.**Simple slopes of the difference between simulated and estimated drift rates at different levels of adversity.
...
The results of our parameter recovery analyses are summarised in Table S3 (preregistered approach) and Table S2 (updated approach).
Using the preregistered approach (simulating the same number of trials as the real data), four out of 16 correlations fell below the pre-specified cut-off (See Table S3).
...
#| results: markup
#| tab.width: 7
staged_sim_results_supp$ddm_fit_table |> 
  border(i = 1, border.top = fp_border_default(style = "none", width = 0), part = "header") |> 
  add_header_row(
    values = " ",
    colwidths = 7
  ...
  compose(
    i = 1, j = 1, 
    as_paragraph(as_b("Table S2. "), "Simulation-based model fit assessment comparing observed and predicted data using 100 simulated trials (accuracy, 25th, 50th, 75th percentile)."),
    part = "header"
  ) |> 
  bold(i = c(3, 4, 8, 10)) |> 
  add_footer_row(
...
#| tab.id: tableS3
#| results: markup
staged_sim_results_supp$ddm_fit_orig_table |> 
  border(i = 1, border.top = fp_border_default(style = "none", width = 0), part = "header") |> 
  add_header_row(
    values = " ",
    colwidths = 7
  ...
  compose(
    i = 1, j = 1, 
    as_paragraph(as_b("Table S3. "), "Simulation-based model fit assessment comparing observed and predicted data using the same number of observed and simulated trials (accuracy, 25th, 50th, 75th percentile)."),
    part = "header"
  ) |> 
  bold(i = c(3, 4, 7, 9)) |> 
  add_footer_row(
...
Table S4 and S5 present correlations between observed and predicted RTs and accuracy at different levels of adversity.
...
#| tab.id: tableS4
#| results: markup
staged_sim_results_supp$ddm_fit_subgroup_dep_table |> 
  border(i = 1, border.top = fp_border_default(style = "none", width = 0), part = "header") |> 
  add_header_row(
    values = " ",
    colwidths = 6
  ...
  compose(
    i = 1, j = 1, 
    as_paragraph(as_b("Table S4. "), "Simulation-based model fit assessment at different levels of material deprivation comparing observed and predicted data using 100 simulated trials (accuracy, 25th, 50th, 75th percentile)."),
    part = "header"
  )
```
...
#| tab.id: tableS5
#| results: markup
staged_sim_results_supp$ddm_fit_subgroup_threat_table |> 
  border(i = 1, border.top = fp_border_default(style = "none", width = 0), part = "header") |> 
  add_header_row(
    values = " ",
    colwidths = 6
  ...
  compose(
    i = 1, j = 1, 
    as_paragraph(as_b("Table S5. "), "Simulation-based model fit assessment at different levels of household threat comparing observed and predicted data using 100 simulated trials (accuracy, 25th, 50th, 75th percentile)."),
    part = "header"
  )
```
...
Although overall accuracy on the Processing Speed Task was very high (`r pcps_clean |> summarise(acc = round(sum(correct)/n()*100,2)) |> pull(acc)`%), overall accuracy for RTs < 1s was below chance, with accuracy increasing above change starting at 1s.
Therefore, we decided to remove RTs < 1s (`r round(sum(pcps_clean$RT < 1)/nrow(pcps_clean)*100,1)`% of trials) and refit the model, which solved the kurtosis in non-decision times.
Recovery of RTs was above the cut-off of .80 for each quantile. 
Thus, we selected this model for the main analyses.

## Model convergence

Figure S13-S16 show model convergence for each task. 
...
#| dpi: 600
#| fig-cap: | 
#|   **Figure S13.** Convergence of the final model for the Mental Rotation Task. Plots should resemble a 'fat, hairy caterpillar'.
staged_sim_results_supp$ddm_conv_lmt_fig
```
...
#| dpi: 600
#| fig-cap: | 
#|   **Figure S14.** Convergence of the final model for the Inhibition Task. Plots should resemble a 'fat, hairy caterpillar'.
staged_sim_results_supp$ddm_conv_flanker_fig
```
...
#| dpi: 600
#| fig-cap: | 
#|   **Figure S15.** Convergence of the final model for the Attention Shifting Task. Plots should resemble a 'fat, hairy caterpillar'.
staged_sim_results_supp$ddm_conv_dccs_fig
```
...
#| dpi: 600
#| fig-cap: | 
#|   **Figure S16.** Convergence of the final model for the Processing Speed Task. Plots should resemble a 'fat, hairy caterpillar'.
staged_sim_results_supp$ddm_conv_pcps_fig
```
...
Figure S17-S20 show the distributions of DDM parameters for each task.
...
#| dpi: 600
#| fig-cap: | 
#|   **Figure S16.** Parameter distributions in the final model of the Mental Rotation Task.
staged_sim_results_supp$ddm_distri_lmt_fig
```
...
#| dpi: 600
#| fig-cap: | 
#|   **Figure S17.** Parameter distributions in the final model of the Inhibition Task.
staged_sim_results_supp$ddm_distri_flanker_fig
```
...
#| dpi: 600
#| fig-cap: | 
#|   **Figure S18.** Parameter distributions in the final model of the Attention Shifting Task.
staged_sim_results_supp$ddm_distri_dccs_fig
```
...
#| dpi: 600
#| fig-cap: | 
#|   **Figure S19.** Parameter distributions in the final model of the Processing Speed Task.
staged_sim_results_supp$ddm_distri_pcps_fig
```
...
The factor loadings and residual variances of the full test model are presented in Table S6. 
Table S7 presents the correlations between latent variables in the model.

```{r}
#| tab.id: tableS6
#| results: markup
staged_sim_results_supp$test_sem_loading_table
```
...
#| tab.id: tableS7
#| results: markup
staged_sim_results_supp$test_sem_cor_table
```
