...
1. We will use Drift Diffusion Modeling (DDM) to investigate how two forms of early-life adversity---threat and deprivation---lower or improve cognitive abilities.
2. The DDM can inform us if and where cognitive differences occur along distinct stages of cognitive processing.
3. We will also use structural equation modeling and out-of-sample cross-validation to tease apart effects of adversity that are task-specific versus task-general.
...
Adversity can lead to either cognitive deficits or enhancements, depending on many factors. 
Though progress has been made, two challenges prevent us from integrating and better understanding these findings. 
First, studies commonly use and interpret raw performance differences. 
However, raw scores conflate different stages of cognitive processing. 
Second, research tends to either isolate or aggregate abilities, which obscures the degree to which individual differences reflect task-specific or task-general processes. 
We address these challenges by using Drift Diffusion Modeling (DDM) and structural equation modeling.
...
This approach holds promise for both deficit and adaptation researchers. 
...
The effects of early-life adverse experiences---such as growing up in poverty or experiencing high levels of violence---on cognition are complex.
...
First, we use modern cognitive modeling that formalizes the stages of processing underlying RTs and accuracy.
...
DDM estimates a set of parameters for each participant that map onto distinct cognitive processes [@voss_2004]. The *drift rate* (*v*) represents the speed of information uptake [@schmiedek_2007; @voss_2013]. 
...
Examples are the time taken to rotate an image on a mental rotation task [@feldman_2021], and updating task rules held in working memory [@schmitz_2012; @schmitz_2014].
...
Though there are studies that focus on changes in DDM parameters in the context of situational threats and anxiety [e.g., @mcfadyen_2022; @tipples_2018; @thompson_2021], no such studies exist in the childhood adversity literature.
...
Thus, the general properties of DDM allow investigating individual differences in drift rates, non-decision times, and boundary separation across a wide range of tasks.
...
Although this is an ongoing debate, it is clear that accounting for task-general processes is critical if we want to build a comprehensive understanding of the developmental effects of early-life on specific abilities.

Structural equation models (SEMs) allow accounting for task-general processes on a latent level. 
...
In this study, we will use the Adolescent Brain Cognitive Development (ABCD) study data ([http://abcdstudy.org](http://abcdstudy.org)) to map key dimensions of adversity to general and task-specific DDM parameters covering attention shifting, inhibition and mental rotation.
...
Next, we excluded participants who 1) had suffered possible mild traumatic brain injury or worse (*n* = `r exclusions$lmt_participant$ex_tbi`); 2) showed a response bias of \> 80% on a task (ranging between *n* = `r min(as.numeric(c(exclusions$lmt_participant$ex_response_biases,exclusions$flanker_participant$ex_response_biases,exclusions$pcps_participant$ex_response_biases,exclusions$dccs_participant$ex_response_biases)))` and `r max(as.numeric(c(exclusions$lmt_participant$ex_response_biases,exclusions$flanker_participant$ex_response_biases,exclusions$pcps_participant$ex_response_biases,exclusions$dccs_participant$ex_response_biases)))`; deviating from the preregistration); 3) had a low number of trials left after trial-level exclusions, defined as \< 20 trials for Mental Rotation and Attention Shifting (*n* = `r exclusions$lmt_participant$ex_below_20_trials` and `r exclusions$dccs_participant$ex_below_20_trials`, respectively) and \< 15 trials for Flanker and Processing Speed (*n* = `r exclusions$flanker_participant$ex_below_15_trials` and `r exclusions$pcps_participant$ex_below_15_trials`, respectively, deviating from the preregistration).
Finally, we excluded task data of several participants based on data inspection (not preregistered): `r exclusions$lmt_participant$ex_0_accuracy` participant with 0% accuracy on the Mental Rotation Task; `r exclusions$pcps_participant$ex_decreasing_effort` participants who showed a sharp decline in accuracy over time on the Processing Speed Task; `r exclusions$dccs_participant$ex_switching_bias` participants on the Attention Shifting Task who (almost) only made switches across all trials, even on repeat trials.
We ended up not excluding participants with missing data one one or more tasks, as the main analyses can account for missing data points.
...
After 8 practice trials, participants completed 30 test trials alternating between shape and color in pseudo-random order.
...
**Household threat.** We will compute threat experienced in the youth's home using the Family Conflict subscale of the ABCD Family Environment Scale [@moos_1994; @zucker_2018].
...
Similar to material deprivation, we will use a previously-created composite score of this measure derived from MNLFA.
...
We will calculate Income-to-needs ratios (INR) by first taking the average of each binned income (\< \$5000, \$5,000--\$11,999, \$12,000--\$15,999, \$16,000--\$24,999, \$25,000--\$34,999, \$35,000--\$49,999, \$50,000--\$74,999, \$75,000--\$99,999, \$100,000--\$199,999, \≥ \$200,000) as a rough approximation of the family's total reported income.
...
For the Flanker and Attention Shifting Task, we will compare model versions that separately estimate drift rate and non-decision-time per task condition or collapse across conditions.
...
**Step 2: Model optimization in training set.** We will first estimate and (where necessary) optimize the SEM using the training set using the *lavaan* package.
This goal of this step is to investigate whether we need to adjust the model specification in any way to achieve good model fit.
For this reason, the model fitted in step 2 will not be interpreted to address our research aims.

See Figure 2 for the *a-priori* specification of the model.
In the measurement model, all three DDM parameters across all tasks (i.e., drift rates, non-decision times, and boundary separations) will be loaded on separate latent factors for each parameter type.
...
The measurement parts of each model will be estimated and optimized step-by-step.
This will allow us to detect sources of potential badness of fit more efficiently.
...
We do not interpret these effects here nor in the final models fitted at step 3.
...
We plan to fit two additional models including neighborhood threat and exposure to severe physical threat as predictors.
Neighborhood threat will be measured using the parent-reported neighborhood safety/crime questionnaire including three items: (a) 'I feel safe walking in my neighborhood', (b) 'Violence is not a problem in my neighborhood', and (c) 'My neighborhood is safe from crime' [@phenx_2016a].
Severe physical threat will be measured with the parent-reported diagnostic interview for DMS-5 relating to traumatic events [@clark_2010].
...
We add these models as secondary analyses because the adversity measures have less desirable measurement characteristics yet are interesting from a conceptual perspective.
Specifically, neighborhood crime is only measured by three items, and endorsement of individual traumatic events items is very low.
A composite score of neighborhood threat will be calculated by averaging the three questionnaire items.
A composite score of severe physical threat will be calculated by summing across all items.
