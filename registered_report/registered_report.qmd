...
#### ^5^ Department of Science, University of Amsterdam, Amsterdam, The Netherlands
...
Based on model fit, we selected the following good-fitting DDM models for the substantive analysis: the base model for the Mental Rotation Task with imputed RTs; the model with one set of parameter estimates across conditions for the Flanker Task and the Attention Shifting Task, and a model with RTs < 1s excluded for the Processing Speed Task (see the supplemental materials for a detailed overview of the results).
For both the Flanker Task and the Attention Shifting Task, estimating separate drift rates and non-decision times per condition resulted in fit indices below our pre-specified cut-offs. 
Thus, we selected the model estimating one drift rate and non-decision time across conditions.
...
Recovery of RTs was above the cut-off of .80 for each quantile (all > XXX), but recovery of accuracy was just below (XXX). 
This seemed due to a ceiling effect in accuracy, and not due to bad fit.
Thus, we selected this model for the main analyses.

## SEM fit

All models provided acceptable to good fit (see Table 1 and the supplemental materials).
We did not---as preregistered---add age as a covariate to the manifest DDM parameters, as age was already included as a covariate for all the latent factors in the context of the MNLFA procedure.
Thus, together with sex, ethnicity, and highest parental education, age effects were accounted for on a latent level.
...
Both material deprivation and household threat showed small, negative correlations with drift rates across all four tasks, indicating that participants with more adversity exposure processed information more slowly.
In addition, both material deprivation and household threat were positively correlated with boundary separation (indicating more response caution), except for Mental Rotation, where we found a small, negative correlation.
Non-decision times were showed either no, or small, negative associations with both material deprivation and household threat.
...
Our primary analysis examined to what extent household threat and material deprivation were associated with task-specific and task-general aspects of information processing (drift rates), response caution (boundary-separations), and task preparation/execution (non-decision times).
...
Both the task-general non-decision time and task-specific non-decision times were practically equivalent at different levels of household threat.

For material deprivation, the task-general drift rate, as well as the task-specific drift rates of the Flanker Task and the Mental Rotation Task, were practically equivalent.
However, we found a significant, negative association between material deprivation and the task-specific drift rate of the Attention Shifting Task ($\beta$ = `r test_reg_coef_list$dccs_v_l_dep_mnlfa$est.std`, 95% CI = [`r test_reg_coef_list$dccs_v_l_dep_mnlfa$ci.lower`, `r test_reg_coef_list$dccs_v_l_dep_mnlfa$ci.upper`], *p* `r test_reg_coef_list$dccs_v_l_dep_mnlfa$pvalue_adj_chr`), indicating that participants with higher levels of material deprivation were slower at shifting attention.
We found a non-significant association with task-general boundary separation, ($\beta$ = `r test_reg_coef_list$a_general_dep_mnlfa$est.std`, 95% CI = [`r test_reg_coef_list$a_general_dep_mnlfa$ci.lower`, `r test_reg_coef_list$a_general_dep_mnlfa$ci.upper`], *p* = `r test_reg_coef_list$a_general_dep_mnlfa$pvalue_adj_chr`), although we also did not find evidence for practical significance. 
Thus, participants with more exposure to material deprivation generally responded with somewhat more caution.
All of the task-specific boundary separations were practically equivalent at different levels of material deprivation.
Both the task-general non-decision time and task-specific non-decision times were practically equivalent at different levels of material deprivation.

Taken together, we did not find task-specific enhanced abilities in line with the adaptation-based framework.
The lowered task-general drift rate was in line with the deficit framework, as is the lowered task-specific drift rate on the Attention Shifting Task.
