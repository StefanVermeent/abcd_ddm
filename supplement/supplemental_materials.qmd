...
The JAGS code will be adjusted to fix the starting point to 0.5 and to estimate parameters per task condition for the Flanker and Attention Shifting Task.
...
Of these samples, every 10th sample will be retained to reduce the size of the model object.
...
The analyses are modeled on the Flanker Task, which is the task with the lowest number of trials.
For simulations involving two conditions, we assume (like we do in the real data) that the drift rate and non-decision time differ (and are correlated) across conditions, and that the boundary separation is the same across conditions.
