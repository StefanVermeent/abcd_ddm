...
For an overview of all milestones, see the [Data Access History](https://github.com/stefanvermeent/abcd_ddm/data_access_history.md).
...
Relative parameter recovery was decent for Boundary Separation (*r* = `ddm_sim1_cor$a`) and Non-Decision Time (*r* = `ddm_sim1_cor$t`), but not for Drift rate (*r* = `ddm_sim1_cor$v`). 
...
As discussed above, the models reported in this manuscript will have more information to go on
